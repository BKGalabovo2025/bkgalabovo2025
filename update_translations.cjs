...
// 1. Tab labels
file = file.replace(
  />\s*Тренировки\s*<\/button>/,
  '> {t("Тренировки", "Trainings", lang)} </button>'
);
file = file.replace(
  />\s*Клубни Услуги\s*<\/button>/,
  '> {t("Клубни Услуги", "Club Services", lang)} </button>'
);
file = file.replace(
  />\s*Магазин\s*<\/button>/,
  '> {t("Магазин", "Shop", lang)} </button>'
);
file = file.replace(
  />\s*Възстановяване\s*<\/button>/,
  '> {t("Възстановяване", "Recovery", lang)} </button>'
);

// 2. Category
file = file.replace(
  />\s*Категория:\s*<\/span>/,
  '> {t("Категория:", "Category:", lang)} </span>'
);
file = file.replace(
  /{cat === "all" \? "Всички" : cat}/,
...
file = file.replace(
  />\s*Изчерпан\s*<\/Badge>/,
  '> {t("Изчерпан", "Out of stock", lang)} </Badge>'
);
file = file.replace(
  />\s*Ограничен \(\{item\.stock\} бр\.\)\s*<\/Badge>/,
  '> {t("Ограничен", "Low stock", lang)} ({item.stock} {t("бр.", "pcs.", lang)}) </Badge>'
);
file = file.replace(
  />\s*В наличност\s*<\/Badge>/,
  '> {t("В наличност", "In stock", lang)} </Badge>'
);
...
file = file.replace(
  ...
  '<span>{item.duration || item.durationMinutes} {t("минути", "minutes", lang)}</span>'
);
file = file.replace(
...
file = file.replace(
  /<span>\s*\{item\.numberOfDays \|\| 1\} дни \/ \{item\.proceduresPerDay \|\| 1\}\s*процедури на ден\s*<\/span>/,
  '<span>{item.numberOfDays || 1} {t("дни", "days", lang)} / {item.proceduresPerDay || 1} {t("процедури на ден", "procedures per day", lang)}</span>'
);

// 7. Price / Button
file = file.replace(
  />\s*Цена\s*<\/span>/,
  '> {t("Цена", "Price", lang)} </span>'
);
file = file.replace(
  /"По заявка"/g,
  'lang === "en" ? "On request" : "По заявка"'
); // Covers both main and modal

// 8. Modal specific strings
file = file.replace(
  />\s*Запиши се \/ Заяви\s*<\/a>/,
  '> {t("Запиши се / Заяви", "Book / Request", lang)} </a>'
);
file = file.replace(
  /Продължителност: /g,
...
file = file.replace(
  / спортисти\s*<\/Badge>/g,
  ' {t("спортисти", "athletes", lang)} </Badge>'
);
file = file.replace(/ дни \/\s*/g, ' {t("дни", "days", lang)} / ');
file = file.replace(
  / процедури на ден\s*<\/Badge>/g,
  ' {t("процедури на ден", "procedures per day", lang)} </Badge>'
);
file = file.replace(
  />\s*Ресурси\s*<\/h4>/,
  '> {t("Ресурси", "Resources", lang)} </h4>'
);
