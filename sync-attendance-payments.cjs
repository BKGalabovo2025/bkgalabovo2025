...
async function syncAttendancePayments() {
  ...
  for (const saleDoc of salesSnap.docs) {
    ...
    const targetMemberIds = sale.memberIdsForAttendance ||
      (sale.memberIdForAttendance ? [sale.memberIdForAttendance] :
        (sale.memberId ? [sale.memberId] : []));

    if (targetMemberIds.length === 0) {
      console.log(`  ⚠️  Продажба ${saleId.substring(0,8).toUpperCase()} — няма целеви членове, пропускам`);
      continue;
    }
    ...
    const paymentType = paymentMode === 'subscription' ? 'subscription' : 'individual';
    const paymentDate = sale.saleDate?.toDate?.()?.toISOString() || sale.saleDate || new Date().toISOString();

    if (targetMonths.length === 0 && targetMonthLabels.length === 0) {
      console.log(`  ⚠️  Продажба ${saleId.substring(0,8).toUpperCase()} — няма целеви месеци, пропускам`);
      continue;
    }

    const memberNames = targetMemberIds.join(', ').substring(0, 40);
    const monthsLabel = targetMonthLabels.length > 0 ? targetMonthLabels.join(', ') : targetMonths.join(', ');
    ...
    for (const memberId of targetMemberIds) {
      ...
      for (const eventDoc of eventsSnap.docs) {
        ...
        const isCovered = targetMonths.includes(eventMonthKey) ||
      ...
      if (memberUpdated > 0) {
        console.log(`   ✅ Член ${memberId.substring(0,8)}: обновени ${memberUpdated} тренировки`);
      } else {
        console.log(`   ℹ️  Член ${memberId.substring(0,8)}: няма нови тренировки за обновяване`);
      }
    }
  }
  ...
  console.log(`   ⏭️  Пропуснати (вече платени/неприсъствали): ${totalSkipped}`);
