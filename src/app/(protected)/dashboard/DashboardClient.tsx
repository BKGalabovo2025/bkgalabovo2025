...
import { getDashboardDataServerAction, invalidateDashboardCacheAction } from "@/lib/actions/dashboard";
...
export default function DashboardClient({ initialData }: DashboardClientProps) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <BentoCard
          className="p-6 flex flex-col justify-between border shadow-none bg-blue-50 text-blue-600 border-blue-100 dark:bg-blue-900/10 dark:text-blue-400 dark:border-blue-800/50 rounded-4xl h-40"
        >
          <div className="flex items-center justify-between mb-2">
            <div className="p-2.5 rounded-2xl bg-blue-100 text-blue-600 dark:bg-blue-800 dark:text-blue-200">
              <Users className="h-5 w-5" strokeWidth={1.5} />
            </div>
            <div className="flex items-center gap-1.5 text-[9px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-full bg-blue-100/80 text-blue-700 dark:bg-blue-900/60 dark:text-blue-300 shadow-none">
              {activeMembers} активни
            </div>
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] mb-2.5 text-blue-600/70 dark:text-blue-400/70">
              {t("dash.active_members")}
            </p>
            {loading ? (
              ...
              <div className="space-y-1 text-zinc-650 dark:text-zinc-350">
                <div className="flex items-center justify-between text-[11px] leading-tight">
                  ...
                  <span className="font-bold text-blue-700 dark:text-blue-300">{stats?.totalClubMembers ?? 0}</span>
                </div>
                <div className="flex items-center justify-between text-[11px] leading-tight">
                  <span className="font-light">Външни клиенти:</span>
                  <span className="font-bold text-amber-700 dark:text-amber-400">{stats?.totalGuests ?? 0}</span>
                </div>
                <div className="flex items-center justify-between text-[11px] leading-tight">
                  <span className="font-light">Семейства:</span>
                  <span className="font-bold text-purple-700 dark:text-purple-400">{stats?.totalFamilies ?? 0}</span>
                </div>
              </div>
            )}
          </div>
        ...
        <BentoCard
          className="p-6 flex flex-col justify-between border shadow-none bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-900/10 dark:text-emerald-400 dark:border-emerald-800/50 rounded-4xl h-40"
        >
          <div className="flex items-center justify-between mb-2">
            <div className="p-2.5 rounded-2xl bg-emerald-100 text-emerald-600 dark:bg-emerald-800 dark:text-emerald-200">
            ...
            {loading ? (
              ...
              <div className="space-y-1 text-zinc-650 dark:text-zinc-350">
                <div className="flex items-center justify-between text-[11px] leading-tight">
                  ...
                  <span className="font-bold text-emerald-700 dark:text-emerald-300">{stats?.todayTrainingsCount ?? 0}</span>
                </div>
                <div className="flex items-center justify-between text-[11px] leading-tight">
                  <span className="font-light">Състезания:</span>
                  <span className="font-bold text-rose-600 dark:text-rose-400">{stats?.todayCompetitionsCount ?? 0}</span>
                </div>
                <div className="flex items-center justify-between text-[11px] leading-tight">
                  <span className="font-light">Лагери & Други:</span>
                  <span className="font-bold text-amber-700 dark:text-amber-400">{(stats?.todayCampsCount ?? 0) + (stats?.todayOtherEventsCount ?? 0)}</span>
                </div>
              </div>
            )}
          </div>
        ...
        <BentoCard
          className="p-6 flex flex-col justify-between border shadow-none bg-purple-50 text-purple-600 border-purple-100 dark:bg-purple-900/10 dark:text-purple-400 dark:border-purple-800/50 rounded-4xl h-40"
        >
          <div className="flex items-center justify-between mb-2">
            ...
            <div className={cn(
              ...
              {revenueTrend >= 0 ? "↗" : "↘"} {revenueTrend > 0 ? "+" : ""}{revenueTrend.toFixed(0)}%
            </div>
          </div>
          <div>
            ...
            {loading ? (
              ...
              <div className="space-y-1 text-zinc-650 dark:text-zinc-350">
                <div className="flex items-center justify-between text-[11px] leading-tight">
                  <span className="font-light">Тренировки:</span>
                  <span className="font-bold text-purple-700 dark:text-purple-300">{formatPrice(stats?.revenueTrainings ?? 0)}</span>
                </div>
                <div className="flex items-center justify-between text-[11px] leading-tight">
                  <span className="font-light">Клубни услуги:</span>
                  <span className="font-bold text-emerald-700 dark:text-emerald-400">{formatPrice(stats?.revenueServices ?? 0)}</span>
                </div>
                <div className="flex items-center justify-between text-[11px] leading-tight">
                  <span className="font-light">Магазин / Стоки:</span>
                  <span className="font-bold text-blue-700 dark:text-blue-400">{formatPrice(stats?.revenueShop ?? 0)}</span>
                </div>
              </div>
            )}
          </div>
        ...
        <BentoCard
          ...
          className={cn(
            "p-6 flex flex-col justify-between border shadow-none rounded-4xl h-40 transition-all duration-300 group cursor-pointer relative overflow-hidden",
          ...
          <div className="relative z-10 flex-1 flex flex-col justify-end">
            ...
            {loading ? (
              ...
              <div className="space-y-1.5 relative">
                {stats?.lowStockProducts?.slice(0, 2).map((p: any) => (
                  <div
                    key={p.id}
                  ...
                  >
                    <span className="font-medium truncate max-w-[100px] text-zinc-700 dark:text-zinc-300" title={p.name}>
                      {p.name}
                    </span>
