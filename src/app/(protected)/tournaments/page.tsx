...
export default function TournamentsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "upcoming": return <Badge variant="secondary">Предстоящ</Badge>;
      case "registration_open": return <Badge variant="default" className="bg-green-600">Отворено записване</Badge>;
      case "ongoing": return <Badge variant="destructive">В ход</Badge>;
      case "completed": return <Badge variant="outline">Приключил</Badge>;
      default: return null;
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex justify-center p-12"><p>Зареждане на турнирите...</p></div>
        ...
        <Card className="flex flex-col items-center justify-center p-12 text-center border-dashed">
          ...
          <Button onClick={() => setIsDialogOpen(true)} variant="outline">Създай първия турнир</Button>
        </Card>
      ) : (
    ...
    <Card className="flex flex-col hover:shadow-md transition-shadow">
      <CardHeader className="pb-3">
        <div className="flex justify-between items-start mb-2">
          ...
          <Badge variant="outline" className="font-normal">{getFormatName(tournament.format)}</Badge>
        </div>
        <CardTitle className="line-clamp-2 leading-tight">{tournament.title}</CardTitle>
      ...
      <CardContent className="flex-1 pb-4">
        ...
        <div className="flex flex-wrap gap-1 mt-4">
          {tournament.categories?.slice(0, 3).map(cat => (
            <span key={cat} className="text-[10px] bg-secondary/50 text-secondary-foreground px-2 py-1 rounded-sm">
              {getCategoryName(cat)}
            </span>
          ))}
