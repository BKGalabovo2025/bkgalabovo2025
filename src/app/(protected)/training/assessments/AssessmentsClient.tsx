    ...
    <>
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 no-print">
          ...
          <Button onClick={handlePrintBlank} variant="outline" className="rounded-xl border-zinc-200">
        ...
        <div className="space-y-12 printable-area bg-white p-2">
          <div className="hidden print:block mb-8 text-center">
            <h1 className="text-2xl font-bold uppercase">Бланка за Оценяване - BWF Тестове</h1>
            <p>Дата: ______________ Треньор: _________________________</p>
          </div>

          {ageGroups.map((group) => {
            const groupTests = BADMINTON_TESTS.filter((t) => t.ageGroup === group);
            ...
            return (
              <div key={group} className="mb-10 page-break-after-always">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {groupTests.map((test) => (
                    <div key={test.id} className="border border-zinc-200 rounded-xl p-5 bg-white shadow-sm flex flex-col justify-between">
                      <div>
                        ...
                        <div className="space-y-2 text-sm bg-zinc-50 p-3 rounded-lg border border-zinc-100">
                          <p><strong className="text-zinc-800">Оборудване:</strong> <span className="text-zinc-600">{test.equipment}</span></p>
                          <p><strong className="text-zinc-800">Оценяване:</strong> <span className="text-zinc-600">{test.scoring}</span></p>
                          <p><strong className="text-zinc-800">Фокус:</strong> <span className="text-zinc-600">{test.focus}</span></p>
                        </div>
                      </div>
                        ...
                        <table className="w-full text-xs text-left border-collapse border border-zinc-300">
                          <thead>
                            <tr className="bg-zinc-100">
                              <th className="border border-zinc-300 p-1 w-1/2">Име на състезател</th>
                              <th className="border border-zinc-300 p-1 w-1/4">Резултат</th>
                              <th className="border border-zinc-300 p-1 w-1/4">Бележка</th>
                            </tr>
                          </thead>
                          <tbody>
                            {[1, 2, 3, 4, 5, 6].map((row) => (
