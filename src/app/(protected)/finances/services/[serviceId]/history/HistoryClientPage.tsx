...
export default function HistoryClientPage({ serviceId }: HistoryClientPageProps) {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-40"><Loader2 className="mr-2 h-8 w-8 animate-spin" /><span>Зареждане на историята...</span></div>
    );
  }
    ...
    return (
      <div className="container mx-auto p-4">
        <div className="p-4 border-l-4 border-red-500 bg-red-50">
          <h2 className="text-xl font-bold text-red-800">Грешка при зареждане</h2>
          <p className="text-red-700 mt-2">{error}</p>
        </div>
        <Link href="/finances/services" className="mt-4 inline-block"><Button variant="outline">Назад към услугите</Button></Link>
      </div>
    );
  }
    ...
    <div className="container mx-auto p-4">
      ...
      {history.length > 0 ? (
        <ul className="space-y-4">
          {history.map(entry => (
            <li key={entry.id} className="p-4 border rounded-lg shadow-sm bg-white">
              <p className="text-sm text-gray-500">{entry.timestamp}</p>
              <p><strong>Потребител:</strong> {entry.userName}</p>
              <p className="mt-2"><strong>Промени:</strong> <span className="text-gray-700">{entry.changes}</span></p>
            </li>
          ))}
        </ul>
      ) : (
