...
const formatDescription = (description: string) => {
    ...
    lines.forEach(line => {
        ...
        if (trimmedLine.startsWith('o\t') || trimmedLine.startsWith('•\t') || trimmedLine.startsWith('-\t') || trimmedLine.startsWith('*')) {
            if (!inList) {
    ...
    return (
        <>
            ...
            <div className="max-w-4xl mx-auto p-8 printable-area font-sans">
                ...
                {/* --- MAIN DESCRIPTION --- */}
                <div className="prose prose-lg max-w-none mb-8" dangerouslySetInnerHTML={formatDescription(service.description)} />
                    ...
                    <div className="mt-10 pt-6 border-t">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {service.targetGroups && service.targetGroups.length > 0 && (
                                <div className="flex items-start"><Users className="h-6 w-6 mr-3 mt-1 text-gray-700"/><p><strong>Целеви групи:</strong> {service.targetGroups.join(', ')}</p></div>
                            )}

                            {service.durationMinutes && (
                                <div className="flex items-start"><Clock className="h-6 w-6 mr-3 mt-1 text-gray-700"/><p><strong>Продължителност:</strong> {service.durationMinutes} минути</p></div>
                            )}
