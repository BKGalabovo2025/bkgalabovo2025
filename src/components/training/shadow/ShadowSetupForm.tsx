            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      const newSettings = { ...settings, ageGroup: a.id as any };
                      if (a.id === "U9-U11") {
                  ...
                  >
                    ...
                    <p className={`text-sm mt-1 font-medium ${settings.ageGroup === a.id ? "text-primary-foreground" : "text-zinc-500"}`}>
                      {a.sub}
                    </p>
                  </button>
                ))}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
                <div className="space-y-1">
                  <Label className="text-zinc-500 text-xs uppercase font-bold">Серии</Label>
                  <Input
                    type="number"
                ...
                <div className="space-y-1">
                  <Label className="text-zinc-500 text-xs uppercase font-bold">
                    {settings.mode === "agility_test" ? "Брой движения" : "Работа (с)"}
                  </Label>
                  <Input
                    type="number"
                    className="font-bold text-lg h-12"
                    value={settings.workSec}
                    onChange={(e) => setSettings({ ...settings, workSec: parseInt(e.target.value) || 30 })}
                  />
                </div>
                <div className="space-y-1">
                  <Label className="text-zinc-500 text-xs uppercase font-bold">Почивка (с)</Label>
                  <Input
                    type="number"
                    className="font-bold text-lg h-12"
                    value={settings.restSec}
                    onChange={(e) => setSettings({ ...settings, restSec: parseInt(e.target.value) || 30 })}
                  />
                </div>
                <div className={`space-y-1 ${settings.mode === "ghost_match" ? "opacity-40 pointer-events-none" : "opacity-100"}`}>
                  <Label className="text-zinc-500 text-xs uppercase font-bold">
                  ...
                  <Input
                    ...
                    onChange={(e) => setSettings({ ...settings, paceSec: parseFloat(e.target.value) || 3 })}
                  />
                </div>
              </div>
            </CardContent>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="space-y-6">
              <Card className="border-2 border-zinc-100 dark:border-zinc-800 shadow-sm h-full flex flex-col">
                <div className="bg-zinc-50 dark:bg-zinc-900/50 p-4 border-b border-zinc-100 dark:border-zinc-800">
                  <h2 className="font-bold text-lg flex items-center gap-2">
                    <RotateCcw size={18} className="text-primary" /> Логистика на корта
                  </h2>
                </div>
                <CardContent className="p-5 flex-1 flex flex-col space-y-5">
                  <div className="space-y-1">
                    <Label className="text-zinc-500 text-xs uppercase font-bold">Налични кортове</Label>
                    <Input
                      type="number"
                      className="font-bold text-lg h-12"
                  ...
                  <div className="space-y-3 pt-4 border-t border-zinc-100 dark:border-zinc-800">
                    <Label className="text-zinc-500 text-xs uppercase font-bold">Експертни Модификатори</Label>
                    <button
                      type="button"
                      role="switch"
                      aria-checked={settings.deceptionEnabled}
                      onClick={() => setSettings({ ...settings, deceptionEnabled: !settings.deceptionEnabled })}
                    ...
                    >
                      <div className="flex flex-col">
                        <span className={`font-bold ${settings.deceptionEnabled ? "text-purple-700 dark:text-purple-400" : "text-zinc-700 dark:text-zinc-300"}`}>Измамни удари (Deception)</span>
                        <span className="text-xs text-zinc-500">Алгоритъм за внезапна смяна</span>
                      </div>
                      <div className={`w-12 h-6 rounded-full transition-colors relative ${settings.deceptionEnabled ? "bg-purple-500" : "bg-zinc-300 dark:bg-zinc-700"}`}>
                        <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${settings.deceptionEnabled ? "left-7" : "left-1"}`} />
                      </div>
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => setSettings({ ...settings, visualOnly: !settings.visualOnly })}
                    ...
                    >
                      <div className="flex flex-col">
                        <span className={`font-bold ${settings.visualOnly ? "text-blue-700 dark:text-blue-400" : "text-zinc-700 dark:text-zinc-300"}`}>Без звук (Visual Only)</span>
                        <span className="text-xs text-zinc-500">Тренира периферното зрение</span>
                      </div>
                      <div className={`w-12 h-6 rounded-full transition-colors relative ${settings.visualOnly ? "bg-blue-500" : "bg-zinc-300 dark:bg-zinc-700"}`}>
                        <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${settings.visualOnly ? "left-7" : "left-1"}`} />
                      </div>
                    </button>
                  </div>
                </CardContent>
            ...
            <div className="space-y-6">
              <Card className="border-2 border-zinc-100 dark:border-zinc-800 shadow-sm h-full flex flex-col">
                <CardContent className="p-5 space-y-4 flex-1 flex flex-col">
                  <Label className="text-sm font-bold uppercase text-zinc-500 tracking-wider">Режим на игра</Label>
                  <div className="flex flex-col gap-2">
                    {[
                      { id: "standard", title: "Стандартен", desc: "Фиксирано таймиране" },
                      { id: "ghost_match", title: "Мач на сенки", desc: "Рандомизирани паузи" },
                      { id: "agility_test", title: "Тест за бързина", desc: "Спринт тест" },
                    ].map((m) => (
                      <button
                        key={m.id}
                        type="button"
                        ...
                        className={`w-full text-left px-4 py-3 rounded-xl border-2 cursor-pointer transition-all flex justify-between items-center ${
                          settings.mode === m.id ? "border-primary bg-primary/10 text-primary" : "border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700"
                        }`}
                      >
              ...
              <Card className="border-2 border-zinc-100 dark:border-zinc-800 shadow-sm h-full flex flex-col">
                <CardContent className="p-5 space-y-4 flex-1 flex flex-col">
                  <div className="flex flex-col gap-3">
                    <Label className="text-sm font-bold uppercase text-zinc-500 tracking-wider">Конфигурация на корта</Label>
                    <div className="flex w-full bg-zinc-100 dark:bg-zinc-900 rounded-lg p-1">
                      <button onClick={() => setSettings({ ...settings, cornersMode: "4-corners" })} className={`flex-1 py-2 text-sm font-bold rounded-md transition-all ${settings.cornersMode === "4-corners" ? "bg-white dark:bg-zinc-800 shadow text-primary" : "text-zinc-500"}`}>4 Ъгъла</button>
                      <button onClick={() => setSettings({ ...settings, cornersMode: "6-corners" })} className={`flex-1 py-2 text-sm font-bold rounded-md transition-all ${settings.cornersMode === "6-corners" ? "bg-white dark:bg-zinc-800 shadow text-primary" : "text-zinc-500"}`}>6 Ъгъла</button>
                    </div>
                  </div>

                  <Label className="text-sm font-bold uppercase text-zinc-500 tracking-wider mt-4 block">Насоченост на зоните</Label>
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      <button
                        key={z.id}
                        onClick={() => setSettings({ ...settings, drillMode: z.id as any })}
                        className={`px-4 py-2 rounded-full text-sm font-bold border transition-all ${
                          settings.drillMode === z.id ? "bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 border-transparent" : "bg-transparent border-zinc-300 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400"
                        }`}
                      >
                        {z.title}
                      </button>
            ...
            <CardContent className="p-5">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 max-h-[300px] overflow-y-auto custom-scrollbar p-1">
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          if (!checked) {
                            setSettings({ ...settings, activePlayers: settings.activePlayers.filter((p) => p.id !== m.id) });
                          } else {
                            setSettings({ ...settings, activePlayers: [...settings.activePlayers, { id: m.id, displayName: `${m.firstName} ${m.lastName}` }] });
                          }
                        }}
                      />
                      <span className={`font-semibold ${isChecked ? "text-zinc-900 dark:text-white" : "text-zinc-500 dark:text-zinc-400"}`}>
                ...
                {initialMembers.length === 0 && (
                  <p className="text-sm text-zinc-500 p-2 text-center mt-4">Няма намерени играчи</p>
                )}
              </div>
            </CardContent>
          </Card>
