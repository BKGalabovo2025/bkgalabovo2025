...
export const GeneralWizardStep3 = () => {
  const {
    step,
    service,
    selectedMember,
      ...
      <div className="space-y-6">
            ...
            <div className="mb-3 text-[9px] flex justify-between items-start bg-zinc-50 dark:bg-zinc-900/50 p-2.5 border border-zinc-100 dark:border-zinc-800 rounded-lg">
              <div>
                ...
                <p className="font-bold uppercase text-zinc-800 dark:text-zinc-200">
                  {selectedMember?.id === "GUEST_EXTERNAL" ? "Външен клиент" : `${selectedMember?.firstName} ${selectedMember?.lastName}`}
                </p>
              </div>
              ...
              <table className="w-full border-collapse border border-zinc-200 dark:border-zinc-800 text-[9px]">
                <thead>
                  <tr className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 text-[8px] font-bold uppercase text-zinc-500">
                    ...
                    <th className="p-1.5 text-center border-r border-zinc-200 dark:border-zinc-800">Кол.</th>
                ...
                <tbody>
                  <tr className="border-b border-zinc-200 dark:border-zinc-800 font-medium">
                    <td className="p-1.5 border-r border-zinc-200 dark:border-zinc-800 font-bold text-left text-zinc-800 dark:text-zinc-200">{service.name}</td>
                    <td className="p-1.5 border-r border-zinc-200 dark:border-zinc-800 text-center">{quantity}</td>
                    <td className="p-1.5 border-r border-zinc-200 dark:border-zinc-800 text-right">{formatPrice(unitPrice)}</td>
            ...
            <div className="mt-4 flex justify-between gap-12 text-zinc-500">
              ...
              <div className="flex-1">
                ...
                <p className="text-[7px] font-bold mt-0.5 uppercase text-center">
                  Получател: {selectedMember?.id === "GUEST_EXTERNAL" ? "Външен клиент" : `${selectedMember?.firstName} ${selectedMember?.lastName}`}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex gap-3 justify-center">
          <Button
            onClick={() => {
              if (completedSaleId) {
                window.open(`/finances/general-services/sales/${completedSaleId}/receipt`, "_blank");
              }
            }}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-zinc-50 dark:bg-zinc-900/40 p-6 rounded-3xl space-y-4 border border-zinc-100/50 dark:border-zinc-900">
        ...
        <div className="flex justify-between items-center text-xs pb-3 border-b border-zinc-200/50 dark:border-zinc-800/50">
          ...
          <span className="font-bold text-zinc-900 dark:text-white">{quantity} x {formatPrice(unitPrice)}</span>
        </div>
        <div className="flex justify-between items-center text-xs pb-3 border-b border-zinc-200/50 dark:border-zinc-800/50">
