          ...
          <div className="space-y-5">
            ...
            <button
              ...
              className={`w-full text-left px-5 py-3.5 flex justify-between items-center transition-colors text-sm font-light rounded-2xl border ${
                isGuestSale
                  ? "bg-amber-50 text-amber-900 border-amber-400/30 dark:bg-amber-950/20 dark:text-amber-300 dark:border-amber-900/35"
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className="font-semibold text-zinc-900 dark:text-zinc-50">
                    Бърза продажба — Външен клиент
                  </span>
                  <span className="text-[10px] text-zinc-500 font-light mt-0.5">
                    Без асоцииране с досие на клубен член
                  </span>
                </div>
              </div>
              {isGuestSale && (
                <Check className="h-4 w-4 text-amber-600 dark:text-amber-500 shrink-0" />
              )}
            </button>
              ...
              <div className="border border-zinc-100 dark:border-zinc-900 rounded-2xl max-h-[240px] overflow-y-auto divide-y divide-zinc-50 dark:divide-zinc-900 custom-scrollbar">
                {filteredMembers.length > 0 ? (
                  filteredMembers.map((member) => {
                    const isSelected =
                      !isGuestSale && selectedMember?.id === member.id;
                    return (
                      <button
                        key={member.id}
                        onClick={() => {
