...
export const GeneralServiceSaleWizardDialog = ({
  ...
  const handleNextStep = () => {
    if (step === 1 && !selectedMember) {
      toast.error("Избор на клиент", {
        description: "Моля, изберете член от списъка или Външен клиент, за да продължите.",
      });
      return;
    }
  ...
  const handleExecuteSale = async () => {
    ...
    try {
      ...
      const clientName = selectedMember.id === "GUEST_EXTERNAL" 
        ? "Външен клиент" 
            ...
            <button
              ...
              className={`w-full text-left px-5 py-3.5 flex justify-between items-center transition-colors text-sm font-light rounded-2xl border ${
                selectedMember?.id === "GUEST_EXTERNAL"
                  ? "bg-amber-50 text-amber-900 border-amber-500/20 dark:bg-amber-950/20 dark:text-amber-300 dark:border-amber-900/35"
                  : "hover:bg-zinc-50 dark:hover:bg-zinc-900/50 text-zinc-800 dark:text-zinc-200 border-zinc-100 dark:border-zinc-900 bg-zinc-50/30 dark:bg-zinc-900/10"
              }`}
            >
              <div className="flex items-center gap-3">
                <div className="p-2 bg-amber-100 dark:bg-amber-500/15 text-amber-600 dark:text-amber-500 rounded-xl">
