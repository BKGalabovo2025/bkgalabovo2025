...
import { getGeneralServiceEvents } from "@/services/general-services";
...
const ServiceHistory = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        setLoading(true);
        const fetchedEvents = await getGeneralServiceEvents();
  ...
  return (
    <div className="bg-white/50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-xl overflow-hidden mt-8">
      <div className="p-8 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/30">
        <h3 className="text-xl font-black font-heading text-zinc-900 dark:text-white">Оперативна история (Услуги)</h3>
        <p className="text-zinc-500 text-sm font-medium mt-1">Пълен одит на промените в каталога с услуги.</p>
      </div>
        ...
        <Table>
          <TableHeader className="bg-zinc-50/80 dark:bg-zinc-800/80">
            <TableRow className="border-zinc-200 dark:border-zinc-800 hover:bg-transparent">
              ...
              <TableHead className="font-bold text-zinc-500 uppercase text-[10px] tracking-widest py-4">Услуга</TableHead>
