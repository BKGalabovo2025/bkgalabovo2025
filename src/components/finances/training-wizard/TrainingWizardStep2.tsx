...
import { CalendarDays, Calendar, CreditCard, Check, CheckSquare, Loader2 } from "lucide-react";
...
import { MonthAttendance } from "./TrainingWizardContext";
...
const TrainingWizardStep2Subscription = ({
  selectedMember,
  monthlyAttendance,
  unpaidMonths,
  selectedMonthKeys,
...
const TrainingWizardStep2Individual = ({
  selectedMember,
  unpaidEvents,
  selectedEventIds,
  setSelectedEventIds,
    ...
    <div className="space-y-3">
      ...
      {unpaidEvents.length === 0 ? (
        <div className="p-8 text-center bg-zinc-50 dark:bg-zinc-900/40 rounded-2xl border border-zinc-100 dark:border-zinc-900">
          ...
          <p className="text-xs font-light text-zinc-400">
            Няма неплатени процедури за {selectedMember.firstName}.
          </p>
        </div>
      ) : (
      ...
      {selectedEventIds.length > 0 && (
        <div className="px-4 py-2.5 bg-blue-50 border border-blue-100 rounded-xl text-[10px] text-blue-700 font-medium">
          Избрани: {selectedEventIds.length} процедури
        </div>
      )}
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      {(() => {
        ...
        if (paymentMode === "subscription") {
          return (
            <TrainingWizardStep2Subscription
