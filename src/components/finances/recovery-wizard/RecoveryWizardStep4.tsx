    ...
    return (
      <div className="py-20 flex flex-col items-center justify-center space-y-6">
        ...
        <div className="text-center space-y-2">
          <p className="font-light text-zinc-900 dark:text-zinc-100 text-lg">Регистриране на продажбата...</p>
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-2 pb-2 border-b border-zinc-100 dark:border-zinc-900">
          ...
          <h3 className="text-sm font-semibold text-zinc-950 dark:text-zinc-50">Касова бележка</h3>
        </div>
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="mb-3 text-[9px] flex justify-between items-start bg-zinc-50 dark:bg-zinc-900/50 p-2.5 border border-zinc-100 dark:border-zinc-800 rounded-lg">
              <div>
                ...
                <p className="font-bold uppercase text-zinc-800 dark:text-zinc-200">{clientDisplayName}</p>
              </div>
              <div className="text-right text-zinc-600 dark:text-zinc-400">
                ...
                <p className="font-bold">Дата: {new Date().toLocaleDateString("bg-BG")} г.</p>
                <p className="mt-0.5">Начин: {paymentMethod}</p>
                <p className="mt-0.5 font-bold">
                  Статус:{" "}
                  <span className={isPaid ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}>
              ...
              <table className="w-full border-collapse border border-zinc-200 dark:border-zinc-800 text-[9px]">
                <thead>
                  <tr className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 text-[8px] font-bold uppercase text-zinc-500">
                    <th className="p-1.5 text-left border-r border-zinc-200 dark:border-zinc-800">Описание</th>
                ...
                <tbody>
                  <tr className="border-b border-zinc-200 dark:border-zinc-800 font-medium">
                    <td className="p-1.5 border-r border-zinc-200 dark:border-zinc-800 font-bold text-left text-zinc-800 dark:text-zinc-200">
                      {service.name}
                      {!isGuestSale && paymentMode === "subscription" && selectedMonthKeys.length > 0 && !selectedMonthKeys.includes("NO_EVENTS") && (
                        <span className="ml-1 font-normal text-zinc-500">({selectedMonthLabels.join(", ")})</span>
                      )}
                      {!isGuestSale && paymentMode === "individual" && selectedEventIds.length > 0 && (
                        <span className="ml-1 font-normal text-zinc-500">
                          ({selectedEventIds.length} процедури
                          {(() => {
                            const dates = selectedEventIds
                              .map((id) => {
                                const ev = memberEvents.find((e) => e.id === id);
                                return ev ? new Date(ev.startDate).toLocaleDateString("bg-BG") : "";
                              })
                              .filter(Boolean);
                            return dates.length > 0 ? ` на ${dates.join(", ")}` : "";
                          })()}
                          )
                        </span>
                      )}
                    </td>
                    <td className="p-1.5 text-right font-bold text-zinc-800 dark:text-zinc-200">{formatPrice(totalAmount)}</td>
                  </tr>
                  <tr>
                    <td className="p-1.5 text-right border-r border-zinc-200 dark:border-zinc-800 font-bold uppercase text-[8px] text-zinc-400">Обща стойност:</td>
            ...
            {note && (
              <div className="mb-3 p-2 border border-dashed border-zinc-300 dark:border-zinc-700 bg-zinc-50/50 dark:bg-zinc-900/30 text-[9px] rounded-lg">
                <span className="font-bold text-zinc-500 uppercase mr-1">Забележка:</span>
                <span className="italic text-zinc-800 dark:text-zinc-200">{note}</span>
              </div>
            )}
            ...
            <div className="mt-4 flex justify-between gap-12 text-zinc-500">
              ...
              <div className="flex-1">
                ...
                <p className="text-[7px] font-bold mt-0.5 uppercase text-center">
                  Получател: {isGuestSale ? "Външен клиент" : `${selectedMember?.firstName} ${selectedMember?.lastName}`}
                </p>
              </div>
            </div>
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-zinc-50 dark:bg-zinc-900/40 p-6 rounded-3xl space-y-4 border border-zinc-100/50 dark:border-zinc-900">
        <div className="flex justify-between items-center text-xs pb-3 border-b border-zinc-200/50 dark:border-zinc-800/50">
          ...
          <span className="font-bold text-zinc-900 dark:text-white">{clientDisplayName}</span>
        </div>
        <div className="flex justify-between items-center text-xs pb-3 border-b border-zinc-200/50 dark:border-zinc-800/50">
          ...
          <>
            <div className="flex justify-between items-center text-xs pb-3 border-b border-zinc-200/50 dark:border-zinc-800/50">
              ...
              <span className="font-bold text-zinc-900 dark:text-white">
                {paymentMode === "subscription" ? "Месечен абонамент" : "Еднократно (Индивидуални процедури)"}
              </span>
            </div>
            {paymentMode === "subscription" && selectedMonthKeys.length > 0 && !selectedMonthKeys.includes("NO_EVENTS") && (
              <div className="flex justify-between items-center text-xs pb-3 border-b border-zinc-200/50 dark:border-zinc-800/50">
                <span className="text-zinc-500">Период</span>
                <span className="font-bold text-emerald-600">{selectedMonthLabels.join(", ")}</span>
              </div>
            )}
            {paymentMode === "individual" && selectedEventIds.length > 0 && (
              <div className="flex justify-between items-center text-xs pb-3 border-b border-zinc-200/50 dark:border-zinc-800/50">
                <span className="text-zinc-500">Брой процедури</span>
                <span className="font-bold text-blue-600">{selectedEventIds.length} процедури</span>
              </div>
            )}
          </>
        )}
        ...
        <div className="flex justify-between items-center text-xs pb-3 border-b border-zinc-200/50 dark:border-zinc-800/50">
          ...
          <span className={`font-bold uppercase tracking-wider text-[10px] ${isPaid ? "text-emerald-600" : "text-rose-600"}`}>
        ...
        {note && (
          <div className="flex flex-col gap-1 text-xs pb-3 border-b border-zinc-200/50 dark:border-zinc-800/50">
            ...
            <span className="font-medium text-zinc-900 dark:text-white italic">{note}</span>
          </div>
        )}
