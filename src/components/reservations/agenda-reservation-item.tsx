          ...
          <div className="flex flex-col gap-1.5 flex-1 min-w-0">
            <div className="flex items-start gap-2">
              ...
              <h4 className="font-bold text-zinc-900 dark:text-white leading-snug break-words w-full flex-1 min-w-0">
                {((reservation as any).memberId || reservation.clientId) ? (
                  <Link 
                    href={`/members/${(reservation as any).memberId || reservation.clientId}`}
                    className="hover:underline hover:text-primary transition-colors"
                  >
                    {reservation.clientName}{reservation.selectedZone ? ` (${reservation.selectedZone})` : ""}
                  </Link>
                ) : (
                  <span>{reservation.clientName}{reservation.selectedZone ? ` (${reservation.selectedZone})` : ""}</span>
                )}
                
                {reservation.client2Name && (
                  <>
                    <span className="mx-1.5 text-zinc-400 font-normal">&amp;</span>
                    {((reservation as any).client2Id) ? (
                      <Link 
                        href={`/members/${(reservation as any).client2Id}`}
                        className="hover:underline hover:text-primary transition-colors"
                      >
                        {reservation.client2Name}{reservation.client2Zone ? ` (${reservation.client2Zone})` : ""}
                      </Link>
                    ) : (
                      <span>{reservation.client2Name}{reservation.client2Zone ? ` (${reservation.client2Zone})` : ""}</span>
                    )}
                  </>
                )}
              </h4>
