    ...
    if (maxQty < 2) {
      return (
        <p className="mt-2 rounded-lg bg-red-500/10 p-2 text-[11px] font-medium text-red-500">
          ...
          {zoneName.toUpperCase()}. Моля, изберете различна зона за Клиент 2.
        </p>
      );
    }
  }
  ...
  useEffect(() => {
    if (!isRecoveryZone && startTime && duration > 0) {
      ...
      if (
        ...
        watchedValues.endTime.getTime() !== newEndTime.getTime()
      ) {
                ...
                {Object.entries(groupedServices).map(
                  ([category, catServices]) => (
                    <div key={category} className="space-y-3">
                      <h3 className="pl-1 text-[10px] font-black tracking-[0.2em] text-zinc-400 uppercase">
                        {category}
                      </h3>
      ...
      {startTime && checkWorkingHours(startTime) && (
        <div className="mt-6 flex items-center justify-between rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-900/50 dark:bg-amber-950/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold tracking-wide text-amber-700 uppercase dark:text-amber-500">
                Извън работно време
              </p>
              <p className="max-w-50 text-[10px] text-amber-600/80 sm:max-w-xs dark:text-amber-600">
                {checkWorkingHours(startTime)}
              </p>
            </div>
          </div>
          ...
          <button
            ...
            onClick={() =>
              setIgnoreWorkingHoursWarning(!ignoreWorkingHoursWarning)
            }
            className={cn(
