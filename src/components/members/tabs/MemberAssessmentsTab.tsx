  ...
  if (assessments.length === 0) {
    return (
      <div className="text-center py-12 bg-zinc-50 border border-zinc-100 rounded-3xl">
        ...
        <h3 className="text-lg font-bold text-zinc-700">Няма данни от тестове</h3>
        <p className="text-sm text-zinc-500 mt-2">
          Започнете да провеждате тестове от меню "Тренировъчен процес -&gt; Тестове".
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      {Object.entries(groupedAssessments).map(([testName, data]) => {
        ...
        return (
          <div key={testName} className="bg-white border border-zinc-100 rounded-3xl p-6">
            ...
            <div className="mt-6 border-t border-zinc-100 pt-4 space-y-3">
              {data.reverse().map((entry) => ( // Reverse just for the list to show newest first
                <div key={entry.id} className="flex justify-between items-center text-sm bg-zinc-50 px-4 py-3 rounded-xl">
                  <div>
                    <span className="font-bold text-zinc-900">{entry.scoreDisplay}</span>
                    {entry.notes && <span className="ml-2 text-zinc-500 italic">- {entry.notes}</span>}
                  </div>
