    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Преглед и филтриране на всички събития, на които членът е присъствал.</CardDescription>
            </CardHeader>
            <CardContent>
                        ...
                        filteredEvents.map(event => (
                            <div key={event.id} className="border p-4 rounded-lg bg-muted/20">
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <Badge variant="secondary">{getBulgarianEventType(event.type)}</Badge>
                                </div>
                                <div className="space-y-2 text-sm text-muted-foreground">
                                    ...
                                    {event.location && <div className="flex items-center"><MapPinIcon className="mr-2 h-4 w-4" /><span>{event.location}</span></div>}
                                    {event.description && <div className="flex items-start"><InfoIcon className="mr-2 h-4 w-4 mt-0.5" /><p className="flex-1">{event.description}</p></div>}
                                </div>
                            </div>
                        ))
                    ) : (
