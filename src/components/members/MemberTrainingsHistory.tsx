...
export function MemberTrainingsHistory({ memberId }: Props) {
  ...
  if (loading) {
    return <div className="p-8 text-center animate-pulse text-slate-400">Зареждане на тренировки...</div>;
  }
  ...
  const totalMinutes = Math.round(trainings.reduce((sum, t) => sum + (t.durationMs || 0), 0) / 60000);
  const badges = [];
  if (totalMinutes > 60) badges.push({ id: "iron", name: "Железни крака", desc: "Над 1 час Shadow Training" });
  if (totalMinutes > 300) badges.push({ id: "lightning", name: "Светкавица", desc: "Над 5 часа Shadow Training" });
    ...
    <div className="space-y-6">
      ...
      {badges.length > 0 && (
        <div className="bg-white border border-zinc-100 rounded-3xl p-6">
          ...
          <div className="flex gap-4">
            {badges.map(b => (
              <div key={b.id} className="p-4 bg-primary/5 border border-primary/20 rounded-xl text-center">
      ...
      <div className="bg-white border border-zinc-100 rounded-3xl p-6">
        <h3 className="text-[11px] font-medium uppercase tracking-widest text-zinc-400 mb-6 flex items-center gap-2">
          <Zap className="h-4 w-4" /> История на тренировките (Общо: {totalMinutes} мин)
        </h3>

        <div className="space-y-4">
          {trainings.map(session => (
            <Card key={session.id} className="border-zinc-100 shadow-none hover:bg-zinc-50 transition-colors">
              <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <div className="font-semibold text-zinc-900">
                    {session.shadowDetails?.mode === "ghost_match" ? "Ghost Match" : 
                     session.shadowDetails?.mode === "agility_test" ? "Скоростен Тест" : "Shadow Training"}
                  </div>
                ...
                <div className="flex items-center gap-6 text-sm text-zinc-600">
                  <div className="text-center">
                    <div className="font-bold">{Math.round((session.durationMs || 0) / 60000)}</div>
