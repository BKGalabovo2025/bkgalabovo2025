...
describe("tournamentService", () => {
  ...
  it("getTournaments should map and return tournaments", async () => {
    ...
    vi.mocked(tournamentRepo.fetchTournaments).mockResolvedValue(mockDocs as any);
  ...
  it("createTournament should throw on invalid schema", async () => {
    ...
    await expect(tournamentService.createTournament(invalidTournament)).rejects.toThrow();
  });
  ...
  it("updateMatchScore should update match and change status to completed", async () => {
    ...
    await tournamentService.updateMatchScore("match1", { score: "2-0" } as any);
    
    expect(tournamentRepo.updateMatchDoc).toHaveBeenCalledWith("match1", expect.objectContaining({
      score: "2-0",
      winnerId: "p1",
      status: "completed"
    }));
