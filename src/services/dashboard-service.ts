...
export const getDashboardStats = (members: Member[], sales: Sale[]) => {
    ...
    const activeMembersCount = safeMembers.filter(m => m && m.status === 'active').length;
    ...
    // --- Sales Analysis ---
    const salesLast30Days = safeSales.filter(s => s && new Date(s.saleDate) >= thirtyDaysAgo);
    const sales30To60DaysAgo = safeSales.filter(s => s && new Date(s.saleDate) >= sixtyDaysAgo && new Date(s.saleDate) < thirtyDaysAgo);

    const salesCountLast30Days = salesLast30Days.length;
    const salesCountPrevious30Days = sales30To60DaysAgo.length;
    const salesChange = salesCountPrevious30Days > 0 
        ? ((salesCountLast30Days - salesCountPrevious30Days) / salesCountPrevious30Days) * 100
    ...
    // --- Member Analysis ---
    const newMembersLast30Days = safeMembers.filter(m => m && new Date(m.registrationDate) >= thirtyDaysAgo).length;
    const newMembers30To60DaysAgo = safeMembers.filter(m => m && new Date(m.registrationDate) >= sixtyDaysAgo && new Date(m.registrationDate) < thirtyDaysAgo).length;
    const newMembersChange = newMembers30To60DaysAgo > 0
        ? ((newMembersLast30Days - newMembers30To60DaysAgo) / newMembers30To60DaysAgo) * 100
        : newMembersLast30Days > 0 ? 100 : 0;
