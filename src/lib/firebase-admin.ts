...
function initializeFirebaseAdmin() {
  ...
  if (process.env.FIREBASE_SERVICE_ACCOUNT_JSON) {
    try {
      ...
      console.log('Firebase Admin SDK initialized securely using FIREBASE_SERVICE_ACCOUNT_JSON.');
    } catch (error) {
      console.error('CRITICAL: Failed to parse FIREBASE_SERVICE_ACCOUNT_JSON or initialize Firebase Admin SDK.', error);
  ...
  // Fallback to the original method (for local file-based development)
     ...
     try {
        ...
        console.error('CRITICAL: Firebase Admin SDK initialization failed with GOOGLE_APPLICATION_CREDENTIALS!', error);
        throw error;
     }
  } 
